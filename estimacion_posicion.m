...
for k=1:N
    ...
    v_std = [ ones(2,1)*gps_std ; ones(6,1)*imu_std ]; %
    delta_k = diag(v_std);
    ...
    H = delta_k * [eye(2) zeros(2,11) ; zeros(3,6) C_a O ; zeros(3,6) C_m O]; % dim(8x13) 6x7
    z_pred = H * x_pred;
    ...
    acc_b = [a_bx(k) a_by(k) a_bz(k)];
    % Magnetometro
    mag_b = [mag_bx(k) mag_by(k) mag_bz(k)];

    m_k = [gps_med_m(k,:) acc_b mag_b]'; % x_gps , y_gps, acc(x,y,z), mag(x,y,z)
    K_k = P_pred * H' * inv(H * P_pred * H' + R_k);
    x_h = x_pred + K_k * (m_k - z_pred);
